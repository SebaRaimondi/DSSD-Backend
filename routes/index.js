...
const setResolver = caseId => new Promise((resolve) => resolvers[caseId] = resolve);
...
router.post('/buy', async (req, res) => {
    ...
    if (req.body.coupon && !coupnum) return res.status(400).json({
        success: false,
        message: 'coupnum must be an integer or not present'
    });
...
router.get('/coupon/:number', async (req, res) => {
...
router.get('/isEmployee/', async (req, res) => {
    ...
    if (!isEmployee) return res.status(500).json({
        email: token.email,
        message: 'Token email is not an employee email',
        success: false
    });
